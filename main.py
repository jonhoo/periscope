...
train_fn = theano.function([learning_rate, flip_var, crop_var, noise, input_var, target_var], loss, updates=updates)
...
while epoch < end:
    ...
    for inp, res in iterate_minibatches(X_train, y_train, args.batchsize, shuffle=True):
        ...
        train_loss += train_fn(learning_rates[epoch], flip, frame, numpy.random.normal(0.0, 0.02, (inp.shape[0], 3, cropsz, cropsz)).astype(theano.config.floatX), inp, res)
        p.update(i)
        i = i+1
        if i > train_batches:
            break
...
def make_response_file(name, fname, cname, X, Y, use_first=False):
    ...
    for index in range(cases):
        ...
        if use_first:
            ...
            toview = Y[index]
        resp_out[index] = debug_fn(probe, nonoise(inp.shape[0]))[:,toview]
        p.update(index + 1)
    del resp_out
    rfile.close()
